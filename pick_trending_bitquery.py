...
def gql(query: str, variables: dict) -> dict:
    if not BEARER.startswith("ory_at_"):
        raise SystemExit("❌ Missing ORY bearer: export BITQUERY_ORY_TOKEN=ory_at_…")
    r = requests.post(
        ENDPOINT,
...
def pick_candidate(newest_minutes: int, newest_count: int, top_since_hours: int, top_count: int):
    ...
    # 3) intersect by mint, prefer entries that have a URI
    ...
    for t in top:
        ...
        if not new: continue
        if not new.get("uri"):  # must have metadata URI
            continue
    ...
    # fallback: first newest with a URI
    if best is None:
        for n in newest:
            if n.get("uri"):
                best = {
...
def main():
    ...
    # Optional: emit env exports you can paste into your shell
